...
Adapting from the OLS examples, it is possible to fit a logistic regression with a loop that repeats calls to a function that returns the OLS coefficients. To do this, the starting point is to transform the data by the logistic link function [@mccullagh1989; @vargas2024]:
\begin{align*}
\mu &= \frac{y + 1/2}{2} \\
...
From the transformed outcome $z$ and a design matrix $X$, we can implement a Re-Weighted Least Squares (RWLS) algorithm to obtain the coefficients of the logistic regression [@mccullagh1989]. The following code is a naive implementation of the RWLS algorithm:
