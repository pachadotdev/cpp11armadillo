...
time of writing this article, there are around 1,100 packages on CRAN that
depend on it.
...
For cases where vectorization is applicable, @burns2011r provides a good
introduction. `cpp11armadillo` is relevant in a project where the same operation is
repeated many times and, at the same time, the computation time saved by using
C++ is greater than the time spent writing and fixing C++ code.
...
FORTRAN was released in 1957, C in 1972, C++ in 1985 and `R` in 1993. All
of these languages have updated releases and standards in the last decade. The
time argument stresses that our examples are mostly instructional, we are not
trying to reinvent the wheel, these examples emerge from the idea of
highlighting the differences that emerge when transitioning from `R` to C++.

There are contemporary versions of the C++ standard, such as C++14,
C++17, and C++20, but C++11 is still widely used. The "11" part in
`cpp11` refers to the fact that it follows the C++11 standard, which was
released in 2011 and introduced several features that make C++ relatively
easier to use and more flexible, despite the fact that it is possible to use it
with codes written using newer C++ standards as long as a compatible compiler
is installed.
...
```cpp
#include "cpp11armadillo/def_blas.hpp"
#include "cpp11armadillo/def_lapack.hpp"
```
...
syntax for the end user. The interested readers can explore @zhang2024openblas
...
matrices filled random values created with `rnorm()`, it takes takes 422
miliseconds to run, while the `pmax()` function takes 7 miliseconds. In other
...
* Step 3: Replace the first column of $U$ with the vector
    ...
    $\| x_1 \|$ is the euclidian norm that is the square root of the sum of the
...
operations, and therefore different complexity. Adapting from @emara2024khufu,
...
which ressemble a list of scalars, vectors or matrices and that are particularly
useful for loops.
...
[@mccullagh_generalized_2019; @vargas2024hitchhikers]:
\begin{align*}
\mu &= \frac{y + 1/2}{2} \\
...
logistic regression [@mccullagh_generalized_2019]. The following code is a naive
...
ECE244 (Programming Fundamentals). `cpp11armadillo` is a byproduct of the knowledge
we acquired in that course.
