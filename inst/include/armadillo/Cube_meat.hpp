...
inline Cube<eT>::Cube(Cube<eT>&& in_cube)
    : n_rows(0),
      ...
      mem_state(0),
      mem() {
  arma_extra_debug_sigprint(arma_str::format("this = %x   in_cube = %x") % this %
                            &in_cube);
...
inline void Cube<eT>::init_cold() {
  arma_extra_debug_sigprint(arma_str::format("n_rows = %u, n_cols = %u, n_slices = %u") %
  ...
  arma_debug_check((((n_rows > 0x0FFF) || (n_cols > 0x0FFF) || (n_slices > 0xFF))
...
inline void Cube<eT>::init_warm(const uword in_n_rows, const uword in_n_cols,
  ...
  arma_extra_debug_sigprint(
      arma_str::format("in_n_rows = %u, in_n_cols = %u, in_n_slices = %u") % in_n_rows %
      in_n_cols % in_n_slices);
    ...
    for (uword s = 0; s < n_slices; ++s) {
      ...
      if (mat_ptr != nullptr) {
        arma_extra_debug_print(
            arma_str::format("Cube::delete_mat(): destroying matrix %u") % s);
        delete mat_ptr;
        mat_ptrs[s] = nullptr;
      }
    }
...
inline Mat<eT>* Cube<eT>::create_mat_ptr(const uword in_slice) const {
  ...
  arma_extra_debug_print(arma_str::format("Cube::create_mat_ptr(): creating matrix %u") %
                         in_slice);
...
arma_inline subview_cube<eT> Cube<eT>::slices(const uword in_slice1,
  ...
  arma_debug_check_bounds((in_slice1 > in_slice2) || (in_slice2 >= n_slices),
...
arma_inline subview_cube<eT> Cube<eT>::tube(const uword in_row1, const uword in_col1) {
  ...
  arma_debug_check_bounds(((in_row1 >= n_rows) || (in_col1 >= n_cols)),
...
inline void Cube<eT>::shed_slice(const uword slice_num) {
  ...
  arma_debug_check_bounds(slice_num >= n_slices,
...
inline void Cube<eT>::shed_slices(const Base<uword, T1>& indices) {
  ...
  if (arma_config::debug) {
    for (uword i = 0; i < N; ++i) {
      arma_debug_check_bounds((slices_to_shed_mem[i] >= n_slices),
...
inline void Cube<eT>::insert_slices(const uword slice_num, const uword N) {
  ...
  arma_debug_check_bounds((slice_num > t_n_slices),
...
inline void Cube<eT>::insert_rows(const uword row_num, const BaseCube<eT, T1>& X) {
  ...
  arma_debug_check(((C.n_cols != n_cols) || (C.n_slices != n_slices)),
...
inline void Cube<eT>::insert_cols(const uword col_num, const BaseCube<eT, T1>& X) {
  ...
  arma_debug_check(((C.n_rows != n_rows) || (C.n_slices != n_slices)),
...
inline void Cube<eT>::insert_slices(const uword slice_num, const BaseCube<eT, T1>& X) {
  ...
  arma_debug_check(((C.n_rows != n_rows) || (C.n_cols != n_cols)),
...
inline bool Cube<eT>::save(const std::string name, const file_type type) const {
  ...
  if (save_okay == false) {
    arma_debug_warn_level(3, "Cube::save(): write failed; file: ", name);
  }
...
inline bool Cube<eT>::save(std::ostream& os, const file_type type) const {
  ...
  if (save_okay == false) {
    arma_debug_warn_level(3, "Cube::save(): stream write failed");
  }
