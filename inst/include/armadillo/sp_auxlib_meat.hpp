...
inline bool sp_auxlib::eigs_sym(Col<eT>& eigval, Mat<eT>& eigvec, const SpBase<eT, T1>& X,
  ...
  if ((arma_config::debug) && (sp_auxlib::rudimentary_sym_check(U.M) == false)) {
    if (is_cx<eT>::no) {
      arma_debug_warn_level(1, "eigs_sym(): given matrix is not symmetric");
    }
    if (is_cx<eT>::yes) {
      arma_debug_warn_level(1, "eigs_sym(): given matrix is not hermitian");
    }
  }
    ...
    if (opts.subdim != 0) {
      if (opts.subdim < (n_eigvals + 1)) {
        arma_debug_warn_level(
            1, "eigs_sym(): opts.subdim must be greater than k; using k+1 instead of ",
            opts.subdim);
        ncv = uword(n_eigvals + 1);
      } else if (opts.subdim > n) {
        arma_debug_warn_level(1,
                              "eigs_sym(): opts.subdim cannot be greater than n_rows; "
                              "using n_rows instead of ",
                              opts.subdim);
        ncv = n;
      } else {
...
inline bool sp_auxlib::eigs_sym_arpack(Col<eT>& eigval, Mat<eT>& eigvec,
...
#if defined(ARMA_USE_ARPACK)
  {
    ...
    // Check for errors.
    if (info != 0) {
      arma_debug_warn_level(1, "eigs_sym(): ARPACK error ", info, " in seupd()");
      return false;
    }
...
#if defined(ARMA_USE_NEWARP)
  {
    ...
    if (opts.subdim != 0) {
      if (opts.subdim < (n_eigvals + 3)) {
        ...
        arma_debug_warn_level(1,
                              "eigs_gen(): opts.subdim cannot be greater than n_rows; "
                              "using n_rows instead of ",
                              opts.subdim);
        ncv = n;
      } else {
...
inline bool sp_auxlib::eigs_gen_arpack(Col<std::complex<T>>& eigval,
...
#if defined(ARMA_USE_ARPACK)
  {
    ...
    // Check for errors.
    if (info != 0) {
      arma_debug_warn_level(1, "eigs_gen(): ARPACK error ", info, " in neupd()");
      return false;
    }
...
inline bool sp_auxlib::spsolve_simple(Mat<typename T1::elem_type>& X,
...
#if defined(ARMA_USE_SUPERLU)
  {
    ...
    arma_debug_check((A.n_rows != X.n_rows),
...
inline bool sp_auxlib::spsolve_refine(Mat<typename T1::elem_type>& X,
...
#if defined(ARMA_USE_SUPERLU)
  {
    ...
    arma_debug_check((A.n_rows != B.n_rows),
                     "spsolve(): number of rows in the given objects must be the same",
                     [&]() { X.soft_reset(); });
    ...
    if ((info > 0) && (info <= int(A.n_cols))) {
      ...
      arma_debug_warn_level(
          2, "spsolve(): system is singular to working precision (rcond: ", rcond, ")");
      status = true;
    } else if (info > int(A.n_cols + 1)) {
      arma_debug_warn_level(1, "spsolve(): memory allocation failure");
    } else if (info < 0) {
      arma_debug_warn_level(1,
                            "spsolve(): unknown SuperLU error code from gssvx(): ", info);
    }
    ...
    while (ido != 99) {
      ...
      switch (ido) {
        ...
        case 1: {
          ...
          // NEW METHOD
          //
          // both operator*(rowvec, sp_mat) and operator*(sp_mat, colvec) can now write to
          // an existing object
...
inline bool superlu_worker<eT>::factorise(typename get_pod_type<eT>::result& out_rcond,
  ...
  if ((l == nullptr) || (u == nullptr)) {
    arma_debug_warn_level(
        3, "superlu_worker()::factorise(): could not construct SuperLU matrix");
    return false;
  }
...
inline bool superlu_worker<eT>::solve(Mat<eT>& X, const Mat<eT>& B) {
  ...
  if (status_XX == false) {
    arma_debug_warn_level(
        3, "superlu_worker()::solve(): could not construct SuperLU matrix");
    return false;
  }
