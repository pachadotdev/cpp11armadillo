...
The previous example, instead of directly inverting $X^tX$, creates empty matrices $Q$ and $R$, and a boolean (e.g., logical) value for `qr_econ()`, and then the QR function tries to decompose $X^tX$ into an orthogonal matrix $Q$ and an upper triangular matrix $R$ such that $XtX = QR$. If the composition is sucessful, the function returns "true" or "false" otherwise. The `solve()` arguments come from the fact that $R$ is upper triangular, and backsolving from the last equation results in $R\beta = QX^ty$.
