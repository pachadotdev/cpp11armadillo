...
packages and it does not offer on-the-fly compilation for code snippets.
...
[RcppArmadillo](https://cran.r-project.org/web/packages/RcppArmadillo/) is a
popular R package used by over 40 CRAN packages as of 2024-12-13. cpp11armadillo
has different internals, even when the goal is the same, and it aims to:
...
`cpp11armadillo`. It includes tests for OLS, eigenvalues, matrix decompositions
(Cholesky and QR), the Capital Asset Pricing Model (CAPM), and data consistency
between R and C++.

The vignette expand on the OLS estimator, and the `econometrics` directory
contains additional examples for the OLS estimator and other statistical models
implemented in C++.
