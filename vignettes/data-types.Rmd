...
* `mat(ptr_aux_mem, n_rows, n_cols, copy_aux_mem = true, strict = false)`
  ...
  * The `strict` parameter comes into effect only when copy_aux_mem is set to
    `false` (ie. the matrix is directly using auxiliary memory).
...
* `vec()`
* `vec(_n_elem_)`
* `vec(_n_elem_, fill_form)` (elements are initialised according to `fill_form`)
* `vec(size(X))`
...
* `vec(ptr_aux_mem, number_of_elements, copy_aux_mem = true, strict = false)`  
  * Create a column vector using data from writable auxiliary (external) memory,
    ...
    directly use the auxiliary memory (ie. no copying). This is faster, but can
    be dangerous unless you know what you are doing.
  * The `strict` parameter comes into effect only when copy_aux_mem is set to
    `false` (ie. the vector is directly using auxiliary memory)
